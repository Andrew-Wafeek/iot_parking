...
public class Parcking {
	...
	public void newCar(Vehicle vehicle) {
		if(slots != 0) {
			if(vehicle.getCarType() == "compact" && (compact > 0 || slots > 0)) {
				for(int i=0;i<16;i++) {
			...
			else if(vehicle.getCarType() == "regular" && regular > 0) {
				for(int i=8;i<16;i++) {
					if(parckedCars.containsKey(i+1) == false) {
		...
		if(tickets.isEmpty()) {
			...
			for(int i=0;i<tickets.size();i++) {
				if(tickets.get(i).getSlot() == slot) {
					if(parckedCars.get(i).getCarType() == "compact") {compact++;} else {regular++;}
					tickets.remove(i);
					//parckedCars.remove(i);
					flag = 1;
	...
	public void printParkedCars() {
		if(slots == 16) {
			...
			for(int i=0;i<16;i++) {
				if(parckedCars.containsKey(i)) {
					System.out.println("-Slot: "+i+" -Car type: "+ parckedCars.get(i).getCarType()+" -car color: "+parckedCars.get(i).getCarColor()+" -car number: "+parckedCars.get(i).getCarNumber());
				}
			}
		}
		System.out.println("\n\n");
